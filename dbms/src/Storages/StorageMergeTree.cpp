...
private:
    /// Must be called under storage.currently_merging_mutex.
    auto getMutationsRange()
    {
        ...
        auto begin_it = storage.current_mutations_by_version.end();
        auto end_it = storage.current_mutations_by_version.end();
        if (sources_data_version != result_data_version)
        {
            begin_it = storage.current_mutations_by_version.upper_bound(sources_data_version);
            end_it = storage.current_mutations_by_version.upper_bound(result_data_version);
        }
...
public:
        ...
        : future_part(future_part_), storage(storage_)
    {
        ...
        auto mutations_range = getMutationsRange();
        ...
        for (const auto & part : future_part.parts)
        {
            for (auto it = mutations_range.first; it != mutations_range.second; ++it)
                it->second.part_name_to_status[part->name].in_progress = true;
        }
    }
    ...
    ~CurrentlyMergingPartsTagger()
    {
        ...
        for (const auto & part : future_part.parts)
        {
            ...
            {
                if (is_successful)
                    it->second.part_name_to_status.erase(part->name);
                else
                {
                    MergeTreeMutationStatus::PartStatus & part_status = it->second.part_name_to_status[part->name];
                    part_status.in_progress = false;
                    part_status.last_exception = exception_message;
                }
            }
        }
    }
