...
void MySQLOutputFormat::initialize()
{
    ...
    for (const auto & type : data_types)
        column_serializers.push_back(&MySQLProtocol::ProtocolBinary::getSerializer(type->getTypeId()));
    ...
    if (header.columns())
    {
        ...
        for (size_t i = 0; i < header.columns(); i++) {
            ...
            packet_sender.sendPacket(column_serializers[i]->getColumnDefinition(column_name, *data_types[i]));
        }
